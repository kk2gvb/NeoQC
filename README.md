...
Вычислительное ядро написано на C++20 и использует параллельную обработку FASTQ-ридов
для ускорения анализа. Визуализация и интерпретируемые статусы
...
- параллельная обработка FASTQ-ридов с использованием OpenMP;
...
Параллельная обработка и оптимизация вычислительного ядра в `v1.0.0`
сокращают среднее время выполнения примерно на **78%** по сравнению с `v0.9.0`.
Результаты анализа при этом остаются неизменными.
