...
Исправленно: сперва создается временное хранилище для всех артефактов и тп, потом уже после успешного окончания расчета заменяет прошлый расчет, за счет чего прошлые данные не сохраняются не при одном из сценариев приведенных в п.5 в TODO.
...
Исправлено: файл case_summary.json создается при каждом запуске, убран захардкоженная переменная `neoqc_version`
...
Исправлено: Проведена проверка реализации чтения quality-строк FASTQ. Для Phred+33 используется преобразование Q = ASCII - 33. Добавлена/проверена валидация допустимого диапазона ASCII [33, 126]. Граничные значения ASCII 33 (!) и ASCII 126 (~) обрабатываются корректно. Значения за пределами диапазона, в частности ASCII 32 и ASCII 127, приводят к ошибке валидации FASTQ с диагностикой некорректного quality-символа.
...
Исправлено: Было разведено на два знаменателя, добалены golden tests
...
Исправлено: реализована length-aware модель распределения GC с распределением наблюдений по соседним bins по логике FastQC. Сохранены оба представления: исходное raw_read_count и FastQC-совместимое fastqc_observed_count. Добавлен regression-тест для последовательностей различной длины.
...
Исправлено:Введён minimum observation count: при количестве наблюдений ≤ 100 percentile-метрики не используются для оценки, а lower quartile и median публикуются как NaN (NOT_EVALUATED).
Для совместимости с выбранной FastQC-like семантикой реализована группировка позиций BaseGroup: для длинных ридов циклы могут объединяться в диапазоны вида 14-15.
Добавлены regression-тесты для проверки порога наблюдений, TSV-контракта одиночных позиций и диапазонов, а также отдельный тест фактического появления BaseGroup-диапазонов. Все тесты проходят.
