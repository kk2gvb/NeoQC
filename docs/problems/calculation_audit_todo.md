...
Исправлено: введён minimum observation count для расчёта percentile-метрик. При недостаточном количестве наблюдений lower quartile и median не используются для оценки и публикуются как nan (NOT_EVALUATED). Для совместимости с выбранной FastQC-like семантикой реализована группировка позиций BaseGroup: циклы могут объединяться в диапазоны вида 14-15. Добавлены тесты как на корректность TSV-контракта с одиночными позициями и диапазонами, так и на фактическое появление BaseGroup-диапазонов. Тесты проходя
