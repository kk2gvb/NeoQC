...
- `minimum_length` рассчитывается по фактически наблюдаемым длинам reads.
- `minimum_length <= 0` приводит к `FAIL` и рассматривается как нарушение
  целостности результата.
- `observed_length_count` показывает количество различных длин reads.
- При `observed_length_count > 1` формируется `WARNING`.
- Variable-length reads намеренно сохраняются как `WARNING`, в том числе
  потому, что различная длина может быть ожидаемым результатом trimming или
  особенностей протокола, но требует внимания при QC.
- Расчёт и правила соответствуют принятой семантике `fastqc-compatible-v1`.

**Проверено:** native FASTQ с фиксированной длиной → PASS; variable-length
FASTQ → WARNING; некорректное значение `minimum_length <= 0` → FAIL.
...
Изначально рассматривалась идея расширить общий TSV-парсер проверками
метрико-специфичных инвариантов.

В текущей реализации каждая метрика имеет собственный формат TSV и
собственную логику обработки/валидации. Универсальное добавление
дополнительных проверок для каждой метрики в общий валидатор приводит к
дублированию логики и не даёт практического преимущества.

Базовая проверка структуры и числовых значений выполняется существующим
механизмом, а специфические требования проверяются на уровне соответствующей
метрики.

Дополнительная переработка общего TSV-валидатора не требуется.
...
**Статус: NOT APPLICABLE / VERIFIED**

Первоначально требовалось усилить воспроизводимость тестовых данных и
проверить идентичность результатов при различном количестве OpenMP-потоков.

Тестовые данные теперь генерируются средствами проекта, поэтому тестовый
набор не зависит от наличия внешних входных файлов.

Корректность параллельного выполнения дополнительно проверялась вручную
при различных значениях количества потоков OpenMP с сопоставлением
получаемых результатов.

Отдельный постоянный differential test для каждого количества потоков
на текущем этапе не добавляется, поскольку требуемая проверка уже
проводилась, а генерация тестовых данных автоматизирована.
